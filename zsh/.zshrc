...
custom_prompt() {
  ...
  case "$cpos" in
    ...
    *)
    ...
    #Are we root?
    [ "$(id -u)" -eq 0 ] && PROMPT+="[%{$fg[white]%}root%{$fg[cyan]%}]%{%G━%}"

    #Current directory
    PROMPT+="[%{$fg[white]%}%{$(truncated_pwd 3)%}%{$fg[cyan]%}]"

    # Background job indicator
    PROMPT+="%(1j.━[%{$reset_color%}⚙%F{1}]%f.)"

    if [ $cmd_cde -eq 0 ]; then
        PROMPT+=$'\n'"%{$fg[cyan]%}┗━━ %F{8}${vim_mode}%f %{$reset_color%}"
    else
        PROMPT+=$'\n'"%{$fg[cyan]%}┗━━ %{$reset_color%}${vim_mode} "
    fi
...
zstyle ':completion:*:descriptions' format '%F{8}■%f %F{cyan}━━━[%f%d%F{cyan}]%f'
zstyle ':completion:*:messages' format '%F{8}■%f %F{cyan}━━━[%f%d%F{cyan}]%f'
...
#export PATH=$PATH:/home/kirill/.spicetify
